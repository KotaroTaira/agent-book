...
class Reflection(BaseModel):
    id: str = Field(description="リフレクション内容に一位性を与えるためのID")
    task: str = Field(description="ユーザーから与えられたタスクの内容")
    reflection: str = Field(description="このタスクに取り組んだ際のあなたの思考プロセスを振り返ってください。何か改善できる点はありましたか?次に同様にタスクに取り組む際に、より良い結果を出すための教訓を2~3分程度で簡潔に述べてください。")
        ...
        with open(self.file_path, "w", encoding="utf-8") as file:
            json.dump(
                [
                    ...
                    for reflection, embedding in zip(self.reflections.values(), self.embeddings_dict.values())
                ],
                file,
                ensure_ascii=False,
                indent=4,
...
class TaskReflector:
    ...
    def run(self, task: str, result: str) -> Reflection:
        prompt = ChatPromptTemplate.from_template(
            ...
            "リフレクション:\n"
            "このタスクに取り組む際に、より良い結果を出すための教訓を2~3文程度で簡潔に述べてください。\n\n"
            "判定:\n"
