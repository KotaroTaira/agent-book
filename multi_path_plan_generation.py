...
class OptionPresenter:
    ...
    def run(self, task: Task):
        ...
        choice_prompt = ChatPromptTemplate.from_template(
            "タスク: 与えらえたタスクとオプションに基づいて、最適なオプションを選択してください。必ず番号のみで回答してください。\n\n"
            "なお、あなたは次の行動しかできません。\n"
            "タスク: {task_name}\n"
            "オプション:\n{options_text}\n"
            "選択: (1-{num_options})\n"
...
class TaskExecutor:
    ...
    def run(self, task: Task, chosen_option: TaskOption, results: list[str]) -> str:
...
def main():
    ...
    parser = argparse.ArgumentParser(
      description="MultiPathPlanGeneratorを使用してタスクを実行します"
    )
