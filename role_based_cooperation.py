...
from single_path_plan_generation import DecomposedTasks, QueryDecomposer
...
class RoleAssigner:
    ...
    def run(self, tasks: list[Task]) -> list[Task]:
        prompt = ChatPromptTemplate(
            [
                (
                    "system", ("あなたは創造的な役割設計の専門家です。与えられたタスクに対して、ユニークで適切な役割を生成してください。")
                ),
                (
                    "human",
                    (
...
class Reporter:
    ...
    def run(self, query: str, results: list[str]) -> str:
        ...
        chian = prompt | self.llm | StrOutputParser()
        return chian.invoke(
            {
                "query": query,
                "results": "\n\n".join(
...
class RoleBasedCooperation:
    def __init__(self, llm: ChatOpenAI):
        ...
        self.role_assigner = RoleAssigner(llm=self.llm)
        self.executor = Executor(llm=self.llm)
        self.reporter = Reporter(llm=self.llm)
        self.graph = self._create_graph()
    
    def _create_graph(self) -> StateGraph:
        workflow = StateGraph(AgentState)
        workflow.add_node("planner", self._plan_tasks)
        workflow.add_node("role_assigner", self._assgin_roles)
    ...
    def _assgin_roles(self, state: AgentState) -> dict[str, Any]:
