...
class ReflectiveAgentState(BaseModel):
    ...
    retry_count: int = Field(default=0, description="タスクの試行回数")
...
class ReflectiveAgent:
    def __init__(
        ...
        self.reflective_goal_creator = ReflectiveGoalCreator(llm=self.llm, reflection_manager=self.reflection_manager)
        self.reflective_response_optimizer = ReflectiveResponseOptimizer(llm=self.llm, reflection_manager=self.reflection_manager)
        self.query_decomposer = QueryDecomposer(llm=self.llm, reflection_manager=self.reflection_manager)
        self.task_executor = TaskExecutor(llm=self.llm, reflection_manager=self.reflection_manager)
        self.result_aggregator = ResultAggregator(llm=self.llm, reflection_manager=self.reflection_manager)
    ...
    def _reflect_on_task(self, state: ReflectiveAgentState) -> dict[str, Any]:
        ...
        return {
            ...
            "retry_count": (
                state.retry_count + 1 if reflection.judgment.needs_retry else state.retry_count
            )
        }
    
    def _shold_retry_or_continue(self, state: ReflectiveAgentState) -> str:
