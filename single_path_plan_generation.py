...
class QueryDecomposer:
    ...
    def run(self, query: str) -> DecomposedTasks:
        prompt = ChatPromptTemplate.from_template(
            ...
            "2. 各タスクは具体的かつ明確に記載されており、単独で実行並びに検証可能な情報を含めること。一切抽象的な表現を含まないこと\n"
...
class SinglePathPlanGeneration:
    ...
    def _execute_task(self, state:SinglePathPlanGenerationState) -> dict[str, Any]:
        ...
        result = self.task_executor.run(current_task, state.results)
        return {
            "results": [result],
...
def main():
    ...
    parser = argparse.ArgumentParser(
      description="SinglePathPlanGeneratorを使用してタスクを実行します"
    )
